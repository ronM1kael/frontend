...
import { View, Text, StyleSheet, TouchableOpacity, Modal, ScrollView, TextInput, Button } from 'react-native';
...
const StudentApplications = () => {
  ...
  }, [context.stateUser.isAuthenticated]); // Trigger fetchStudentApplications on login/logout
  ...
  const fetchStudentApplications = async () => {
    try {
      ...
      // Clear applications when a new user logs in
      setApplications([]);
      ...
      response.data.application.forEach(application => {
  ...
  const handleTechnicalAdviserApproval = async () => {
    try {
      const jwtToken = await AsyncStorage.getItem('jwt');
      const response = await axios.post(`your_backend_url_here/${selectedApplication.id}`, {
